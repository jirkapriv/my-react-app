...
export default function CarView() {
  ...
  const handleChange = (e) => {
    //pokud do vstupu bude psat nejaky text tak se bude dit event(e)
  ...
  const handleDelete = async (e) => {
    ...
    if (car.name === formData) {
      //pokud se jmeno shoduje
      const data = await deleteCar(id); //
      if (data.status === 200) {
        //pokud je vse ok tak se stane funkce
        redirectToMainPage();
      } else {
        //pokud neni spravne tak to vypise zpravu chyby
        setInfo(data.msg);
      }
    } else {
      //pokud se neshoduje
